...
def load_trained_models(decoding='PerceptionImagination'):
    ...
    for model in models:
        ...
        if model.endswith('.pkl'):
            loadedModels.append(load_models(modelPath, type='.pkl'))
        else:
            loadedModels.append(load_models(modelPath))

    return loadedModels, modelNames

def load_models(modelPath, type='Tensor'):
    if type != 'Tensor':
